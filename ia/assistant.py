...
def repondre_question(question, contexte_cours=None):
    ...
    if cours_pertinents:
        chapitre = cours_pertinents[0]["chapitre"]
        extrait = cours_pertinents[0]["contenu"][:300]

        reponse = (
            f"D'après le chapitre **{chapitre}**, voici ce que je peux te dire :\n\n"
            f"{extrait}...\n\n"
            f"Consulte le cours complet dans la section « Cours » pour plus de détails."
        )
    else:
        reponse = (
            "Je n'ai pas trouvé de cours correspondant précisément à ta question. "
            "Essaie de reformuler, ou consulte le menu « Cours » pour parcourir les chapitres disponibles."
        )
